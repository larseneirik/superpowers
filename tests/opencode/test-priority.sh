...
# Verifies that skills are resolved with correct priority: project > personal > codexpowers
...
# 1. Create in codexpowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/codexpowers/skills/priority-test"
cat > "$HOME/.config/opencode/codexpowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: CodexPowers version of priority test skill
---
# Priority Test Skill (CodexPowers Version)

This is the CODEXPOWERS version of the priority test skill.

PRIORITY_MARKER_CODEXPOWERS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/codexpowers/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] CodexPowers version exists"
else
    echo "  [FAIL] CodexPowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides codexpowers
echo ""
echo "Test 2: Testing personal > codexpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides codexpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_CODEXPOWERS_VERSION"; then
    echo "  [FAIL] CodexPowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|codexpowers" | head -10
fi

# Test 3: Test that project overrides both personal and codexpowers
echo ""
echo "Test 3: Testing project > personal > codexpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] CodexPowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit codexpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing codexpowers: prefix forces codexpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load codexpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_CODEXPOWERS_VERSION"; then
    echo "  [PASS] codexpowers: prefix correctly forces codexpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] codexpowers: prefix did not force codexpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
